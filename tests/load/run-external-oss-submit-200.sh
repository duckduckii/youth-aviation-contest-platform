...
SCENARIO_MODE="${SCENARIO_MODE:-per-vu}"
TOTAL_ITERATIONS="${TOTAL_ITERATIONS:-$USER_COUNT}"
LOGIN_RETRY_MAX="${LOGIN_RETRY_MAX:-3}"
LOGIN_RETRY_BACKOFF="${LOGIN_RETRY_BACKOFF:-3}"
FLOW_RETRY_MAX="${FLOW_RETRY_MAX:-1}"
REPORT_DIR="${REPORT_DIR:-$ROOT_DIR/reports/external-oss-submit-200-$(date '+%Y%m%d-%H%M%S')}"

if [ -z "$BASE_URL" ]; then
  echo "BASE_URL is required, for example:" >&2
  echo "  BASE_URL=http://your-public-host:80 bash tests/load/run-external-oss-submit-200.sh" >&2
  exit 1
fi
